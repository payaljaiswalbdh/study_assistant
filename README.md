...
Browser (React + Vite :5173)
    │
    ├── /api/generate ──► Express Proxy (:3001) ──► Gemini API
    │                       └── responseSchema enforces JSON shape
    │
    ├── validateAndCoerce() ── catches malformed AI output
    │
    └── localGenerate() ────── fallback if API is unavailable
```
